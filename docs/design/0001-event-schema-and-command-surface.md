...
Status: accepted (Increment 0, GWLJ-rk2cnb)
...
| --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `gwl-jobs ingest <url> [--source <s>]`                                                  | Fetch, extract, dedupe, gate, score a posting; `--source` records how it was found (default `unknown`).                                                                                                                                                                                                                                       | `ingested` / `updated` / `reingest_suppressed`, then `rejected` or `scored` |
| `gwl-jobs ingest --file <path> [--source <s>]`                                          | Same, from a local HTML/text drop.                                                                                                                                                                                                                                                                                                            | same                                                                        |
| `gwl-jobs list [--all]`                                                                 | Print the active pipeline: every non-terminal, non-ignored lead, ranked by score (design doc 0002). Each row shows rank, composite score, title @ company, deferral count (when nonzero), lifecycle-status tag, and the lead's UUID prefix (the addressing handle — design doc 0002, decision 0008). `--all` adds terminal and ignored leads. | —                                                                           |
| `gwl-jobs review`                                                                       | Interactive prompt loop (§5).                                                                                                                                                                                                                                                                                                                 | `reviewed`, `apply_queued`                                                  |
| `gwl-jobs mark <lead> <mark> [--note] [--json]`                                         | Non-interactive mark (scriptable). `apply-automatically` runs the same prepare → batch-append → open flow as the review loop's `a` key (§5). Human output is one line (`marked <prefix> <mark>`); `--json` prints the summary (lead id, mark, and the prepared package).                                                                      | `reviewed` (+ `apply_queued`)                                               |
...
| `gwl-jobs applied <lead> [--method <m>] [--note] [--at <ts>]`                           | Record the `applied` transition. `--method` defaults from the lead's apply mark (design doc 0002).                                                                                                                                                                                                                                            | `applied`                                                                   |
| `gwl-jobs screened <lead> [--contact <c>] [--note] [--at <ts>]`                         | Record the `screened` transition.                                                                                                                                                                                                                                                                                                             | `screened`                                                                  |
| `gwl-jobs interviewed <lead> [--stage <s>] [--note] [--at <ts>]`                        | Record the `interviewed` transition.                                                                                                                                                                                                                                                                                                          | `interviewed`                                                               |
| `gwl-jobs offered <lead> [--note] [--at <ts>]`                                          | Record the `offered` transition.                                                                                                                                                                                                                                                                                                              | `offered`                                                                   |
| `gwl-jobs outcome <lead> <type> [--note] [--at <ts>] [--start-date <d>] [--reason <r>]` | Record a terminal outcome (§3): `accepted` / `rejected_by_employer` / `withdrawn` / `declined` / `unresponsive` / `archived`. `--start-date` is `accepted`-only; `--reason` is `archived`-only.                                                                                                                                               | the terminal event                                                          |
